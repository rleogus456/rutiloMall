$(function(){
	// 사이드 메뉴 탭 여닫기
	$("#layout_side div.menu_navigation_wrap ul.tab li").click(function(){
		var tabIdx = $("#layout_side div.menu_navigation_wrap ul.tab li").index(this);
		$("#layout_side div.menu_navigation_wrap ul.tab li").removeClass('current');
		$(this).addClass('current');
		$("#layout_side div.menu_navigation_wrap ul.menu").hide().eq(tabIdx).show();
	});

	// 사이드 메뉴 여닫기
	$("#layout_side div.menu_navigation_wrap ul.menu li.mitem a.mitem_title").click(function(){
	...
	$("#layout_side .mitem_favorite").click(function(){
		...
		switch(ctype){
			case 'category'	: var ctype_name = '카테고리'; break;
			case 'brand'	: var ctype_name = '브랜드'; break;
			default : var ctype_name = '카테고리'; break;
		}
		...
		$.ajax({
			...
			'success' : function(res){
				if(res=='on') {
					...
					$("#category_favorite_alert .cfa_msg").html("즐겨찾는 "+ctype_name+"에<br />저장되었습니다.");
					...
					$("#category_favorite_alert .cfa_msg").html("즐겨찾는 "+ctype_name+"에서<br />삭제되었습니다.");
				}else{
	...
	var brandsort = function(kind1,word){
		...
		if(kind1==2 || word){
			$("#layout_side .mitem_brand").each(function(){
				...
				// 가나다순
				if(kind1==0){
					if(word=='ㄱ' && title.substring(0,1).search(/[가-깋]/)!=-1) {$(this).show(); $(".mitem_title",this).text(title);}
					if(word=='ㄴ' && title.substring(0,1).search(/[나-닣]/)!=-1) {$(this).show(); $(".mitem_title",this).text(title);}
					if(word=='ㄷ' && title.substring(0,1).search(/[다-딯]/)!=-1) {$(this).show(); $(".mitem_title",this).text(title);}
					if(word=='ㄹ' && title.substring(0,1).search(/[라-맇]/)!=-1) {$(this).show(); $(".mitem_title",this).text(title);}
					if(word=='ㅁ' && title.substring(0,1).search(/[마-밓]/)!=-1) {$(this).show(); $(".mitem_title",this).text(title);}
					if(word=='ㅂ' && title.substring(0,1).search(/[바-빟]/)!=-1) {$(this).show(); $(".mitem_title",this).text(title);}
					if(word=='ㅅ' && title.substring(0,1).search(/[사-싷]/)!=-1) {$(this).show(); $(".mitem_title",this).text(title);}
					if(word=='ㅇ' && title.substring(0,1).search(/[아-잏]/)!=-1) {$(this).show(); $(".mitem_title",this).text(title);}
					if(word=='ㅈ' && title.substring(0,1).search(/[자-짛]/)!=-1) {$(this).show(); $(".mitem_title",this).text(title);}
					if(word=='ㅊ' && title.substring(0,1).search(/[차-칳]/)!=-1) {$(this).show(); $(".mitem_title",this).text(title);}
					if(word=='ㅋ' && title.substring(0,1).search(/[카-킿]/)!=-1) {$(this).show(); $(".mitem_title",this).text(title);}
					if(word=='ㅌ' && title.substring(0,1).search(/[타-팋]/)!=-1) {$(this).show(); $(".mitem_title",this).text(title);}
					if(word=='ㅍ' && title.substring(0,1).search(/[파-핗]/)!=-1) {$(this).show(); $(".mitem_title",this).text(title);}
					if(word=='ㅎ' && title.substring(0,1).search(/[하-힣]/)!=-1) {$(this).show(); $(".mitem_title",this).text(title);}
				}
				...
				if(kind1==1){
					if(title.substring(0,1)==word){
						$(this).show();
						$(".mitem_title",this).text(title_org);
					}else if(title_eng.substring(0,1)==word){
						$(this).show();
						$(".mitem_title",this).text(title_eng_org);
					}
				}
	...
	$("#layout_side .brandsort>li").eq(0).trigger('click');
});